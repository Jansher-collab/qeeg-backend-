"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.GET = GET;
exports.POST = POST;
const server_1 = require("next/server");
const prisma_1 = require("@/lib/backend/prisma");
const authService_1 = require("@/lib/backend/services/authService");
const activityLogger_1 = require("@/lib/backend/services/activityLogger");
const client_1 = require("@/generated/prisma/client");
async function GET(req, { params }) {
    const action = params.action?.[0];
    if (action === 'me') {
        return handleMe(req);
    }
    return server_1.NextResponse.json({ error: 'Not Found' }, { status: 404 });
}
async function POST(req, { params }) {
    const action = params.action?.[0];
    switch (action) {
        case 'login':
            return handleLogin(req);
        case 'logout':
            return handleLogout(req);
        case 'signup':
            return handleSignup(req);
        case 'forgot-password':
            return handleForgotPassword(req);
        case 'reset-password':
            return handleResetPassword(req);
        default:
            return server_1.NextResponse.json({ error: 'Not Found' }, { status: 404 });
    }
}
async function handleMe(req) {
    try {
        const user = await (0, authService_1.getAuthenticatedUser)(req);
        if (!user) {
            return server_1.NextResponse.json({ authenticated: false, user: null }, { status: 401 });
        }
        return server_1.NextResponse.json({
            authenticated: true,
            user: {
                id: user.id,
                email: user.email,
                role: user.role,
                practitionerProfile: user.practitionerProfile,
            },
        });
    }
    catch (error) {
        console.error('Error verifying auth session:', error);
        return server_1.NextResponse.json({ authenticated: false, user: null }, { status: 500 });
    }
}
async function handleLogin(req) {
    try {
        const body = (await req.json());
        const { email, password } = body;
        if (!email || !password) {
            return server_1.NextResponse.json({ error: 'Email and password are required.' }, { status: 400 });
        }
        const user = await prisma_1.prisma.user.findUnique({
            where: { email: email.toLowerCase().trim() },
            include: {
                practitionerProfile: true,
            },
        });
        if (!user) {
            return server_1.NextResponse.json({ error: 'Invalid email or password.' }, { status: 401 });
        }
        const isValidPassword = await (0, authService_1.verifyPassword)(password, user.passwordHash);
        if (!isValidPassword) {
            return server_1.NextResponse.json({ error: 'Invalid email or password.' }, { status: 401 });
        }
        const token = (0, authService_1.generateToken)({
            userId: user.id,
            email: user.email,
            role: user.role,
        });
        const ip = req.headers.get('x-forwarded-for') || '127.0.0.1';
        await (0, activityLogger_1.logActivity)({
            userId: user.id,
            action: 'USER_LOGIN',
            details: { role: user.role },
            ipAddress: ip,
        });
        const response = server_1.NextResponse.json({
            message: 'Login successful.',
            user: {
                id: user.id,
                email: user.email,
                role: user.role,
                practitionerProfile: user.practitionerProfile,
            },
        });
        const cookieOptions = (0, authService_1.getSessionCookieOptions)();
        response.cookies.set(cookieOptions.name, token, cookieOptions);
        return response;
    }
    catch (error) {
        console.error('Error during login:', error);
        return server_1.NextResponse.json({ error: error.message || 'Failed to login.' }, { status: 500 });
    }
}
async function handleLogout(req) {
    const response = server_1.NextResponse.json({ message: 'Logged out successfully.' });
    const cookieName = (0, authService_1.getSessionCookieName)();
    response.cookies.set(cookieName, '', {
        httpOnly: true,
        secure: process.env.NODE_ENV === 'production',
        sameSite: 'lax',
        path: '/',
        maxAge: 0,
    });
    return response;
}
async function handleSignup(req) {
    try {
        const body = (await req.json());
        const { email, password, role = 'PRACTITIONER', fullName, professionalTitle, profession, providerNumber, clinicName, practiceAddress, phone, practiceEmail, notificationEmail, } = body;
        if (!email || !password) {
            return server_1.NextResponse.json({ error: 'Email and password are required.' }, { status: 400 });
        }
        if (password.length < 8) {
            return server_1.NextResponse.json({ error: 'Password must be at least 8 characters long.' }, { status: 400 });
        }
        const existingUser = await prisma_1.prisma.user.findUnique({
            where: { email: email.toLowerCase().trim() },
        });
        if (existingUser) {
            return server_1.NextResponse.json({ error: 'An account with this email address already exists.' }, { status: 409 });
        }
        const passwordHash = await (0, authService_1.hashPassword)(password);
        const assignedRole = role === 'NEUROSCIENTIST' ? client_1.UserRole.NEUROSCIENTIST : client_1.UserRole.PRACTITIONER;
        const newUser = await prisma_1.prisma.user.create({
            data: {
                email: email.toLowerCase().trim(),
                passwordHash,
                role: assignedRole,
                practitionerProfile: {
                    create: {
                        fullName: fullName?.trim() || null,
                        professionalTitle: professionalTitle?.trim() || null,
                        profession: profession?.trim() || null,
                        providerNumber: providerNumber?.trim() || null,
                        clinicName: clinicName?.trim() || null,
                        practiceAddress: practiceAddress?.trim() || null,
                        phone: phone?.trim() || null,
                        practiceEmail: practiceEmail?.trim() || null,
                        notificationEmail: notificationEmail?.trim() || email.toLowerCase().trim(),
                    },
                },
            },
            include: {
                practitionerProfile: true,
            },
        });
        const token = (0, authService_1.generateToken)({
            userId: newUser.id,
            email: newUser.email,
            role: newUser.role,
        });
        const ip = req.headers.get('x-forwarded-for') || '127.0.0.1';
        await (0, activityLogger_1.logActivity)({
            userId: newUser.id,
            action: 'PRACTITIONER_REGISTERED',
            details: {
                role: newUser.role,
                profession: newUser.practitionerProfile?.profession,
                clinicName: newUser.practitionerProfile?.clinicName,
            },
            ipAddress: ip,
        });
        const response = server_1.NextResponse.json({
            message: 'Account created successfully.',
            user: {
                id: newUser.id,
                email: newUser.email,
                role: newUser.role,
                practitionerProfile: newUser.practitionerProfile,
            },
        }, { status: 201 });
        const cookieOptions = (0, authService_1.getSessionCookieOptions)();
        response.cookies.set(cookieOptions.name, token, cookieOptions);
        return response;
    }
    catch (error) {
        console.error('Error during signup:', error);
        return server_1.NextResponse.json({ error: error.message || 'Failed to create account.' }, { status: 500 });
    }
}
async function handleForgotPassword(req) {
    try {
        const body = (await req.json());
        const { email } = body;
        if (!email) {
            return server_1.NextResponse.json({ error: 'Email address is required.' }, { status: 400 });
        }
        const normalizedEmail = email.toLowerCase().trim();
        const user = await prisma_1.prisma.user.findUnique({
            where: { email: normalizedEmail },
        });
        const clientIp = req.headers.get('x-forwarded-for') || '127.0.0.1';
        if (!user) {
            return server_1.NextResponse.json({
                message: 'If an account exists with this email, password reset instructions have been generated.',
            });
        }
        const token = await (0, authService_1.createPasswordResetToken)(normalizedEmail);
        const origin = req.nextUrl.origin || 'http://localhost:3000';
        const resetUrl = `${origin}/reset-password?token=${token}`;
        await (0, activityLogger_1.logActivity)({
            userId: user.id,
            action: 'PASSWORD_RESET_REQUESTED',
            details: { email: normalizedEmail },
            ipAddress: clientIp,
        });
        return server_1.NextResponse.json({
            message: 'Password reset link generated successfully.',
            resetUrl,
            token,
        });
    }
    catch (error) {
        console.error('Error generating password reset token:', error);
        return server_1.NextResponse.json({ error: error.message || 'Failed to process password reset request.' }, { status: 500 });
    }
}
async function handleResetPassword(req) {
    try {
        const body = (await req.json());
        const { token, newPassword } = body;
        if (!token || !newPassword) {
            return server_1.NextResponse.json({ error: 'Token and new password are required.' }, { status: 400 });
        }
        if (newPassword.length < 8) {
            return server_1.NextResponse.json({ error: 'Password must be at least 8 characters long.' }, { status: 400 });
        }
        const tokenRecord = await (0, authService_1.validatePasswordResetToken)(token);
        if (!tokenRecord) {
            return server_1.NextResponse.json({ error: 'This password reset link is invalid or has expired.' }, { status: 400 });
        }
        const user = await prisma_1.prisma.user.findUnique({
            where: { email: tokenRecord.email },
        });
        if (!user) {
            return server_1.NextResponse.json({ error: 'User account associated with this token was not found.' }, { status: 404 });
        }
        const passwordHash = await (0, authService_1.hashPassword)(newPassword);
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.user.update({
                where: { id: user.id },
                data: { passwordHash },
            }),
            prisma_1.prisma.passwordResetToken.update({
                where: { id: tokenRecord.id },
                data: { used: true },
            }),
        ]);
        const clientIp = req.headers.get('x-forwarded-for') || '127.0.0.1';
        await (0, activityLogger_1.logActivity)({
            userId: user.id,
            action: 'PASSWORD_RESET_COMPLETED',
            details: { email: tokenRecord.email },
            ipAddress: clientIp,
        });
        return server_1.NextResponse.json({
            message: 'Password reset successfully. You may now log in with your new password.',
        });
    }
    catch (error) {
        console.error('Error resetting password:', error);
        return server_1.NextResponse.json({ error: error.message || 'Failed to reset password.' }, { status: 500 });
    }
}
