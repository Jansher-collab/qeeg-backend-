...
export interface DomainFinding {
  ...
  literatureCitations: LiteratureResult[];
}
...
export const MANDATORY_NEUROFEEDBACK_COI_DISCLOSURE =
  'Conflict of Interest Disclosure: Neurofeedback treatment options presented herein are based on published literature and protocol standardisations. The platform maintains no financial interest, sponsorship, or commercial endorsement of specific equipment manufacturers, software vendors, or proprietary neurofeedback systems.';

export const MANDATORY_AI_GENERATION_DISCLAIMER =
  'Disclaimer: This report was generated using automated analytical correlation pipelines and synthesized literature databases. It is intended strictly as a decision-support tool for licensed practitioners and does not constitute formal medical diagnosis or individual clinical advice.';
 ...
 */
export async function compileCorrelationReport(
  caseReference: string,
  age?: number,
  gender?: string,
  handedness?: string,
...
): Promise<CompiledReportFindings> {
  ...
  const literatureResults = await aggregatedLiteratureService.searchLiterature(literatureQuery, 4);

  // 1. Medical Domain
  const medicalLit = await aggregatedLiteratureService.searchLiterature('QEEG neurological correlates EEG abnormalities', 2);
  const medicalDomain: DomainFinding = {
    domain: 'Medical',
  ...
  const psychLit = await aggregatedLiteratureService.searchLiterature('TOVA attention executive function QEEG correlates', 2);
  const psychologicalDomain: DomainFinding = {
  ...
  const nutritionLit = await aggregatedLiteratureService.searchLiterature('nutrition neurochemistry EEG brain function co-factors', 2);
  const nutritionalDomain: DomainFinding = {
    domain: 'Nutritional',
  ...
  // 4. Lifestyle Domain
  const lifestyleLit = await aggregatedLiteratureService.searchLiterature('sleep hygiene exercise QEEG brain wave modulation', 2);
  const lifestyleDomain: DomainFinding = {
    domain: 'Lifestyle',
  ...
  const nfbLit = await aggregatedLiteratureService.searchLiterature('neurofeedback protocol Theta Beta SMR down-training clinical response', 2);
  const neurofeedbackDomain: DomainFinding = {
    ...
    conflictOfInterestDisclosure: MANDATORY_NEUROFEEDBACK_COI_DISCLOSURE,
